...
class Student:
    def __init__(self,root):
       ...
       #1st image 
        ...
        img=img.resize((1550,170), Image.LANCZOS)
        ...
        img_5=Image.open(r'./college_images/3.jpeg')
        ...
        upload_button = Button(left_frame, text="Upload Image", command=self.upload_image, font=("Arial", 12, "bold"), bg="blue", fg="white")
        upload_button.grid(row=4, column=3, padx=2, pady=7, sticky=W)
        ...
        #img 1
        img_7=Image.open(r'./college_images/5th.gif')
        img_7=img_7.resize((798,200),Image.LANCZOS)
        self.photoimg_7=ImageTk.PhotoImage(img_7)

        
        
        self.right_img=Label(right_frame,image=self.photoimg_7,bd=2,relief=RIDGE)
        self.right_img.place(x=0,y=0,width=798,height=200)
    ...
    def upload_image(self):
        ...
        # You can do additional processing with the selected image file here
    ...
    def add_data(self):
        if (self.var_enrollment.get() == "" or self.var_name.get() == "" or self.var_course.get() == ""):
            ...
            try:
                ...
                db_name = os.environ.get('student_management_system')
                conn = mysql.connector.connect(host="localhost", username="root", password="root",
                                                database="student_management_system")
                my_cursur = conn.cursor(prepared=True)
                ...
                my_cursur.execute(query, values)
                conn.commit()
                self.fetch_data()
                conn.close()
                ...
                # Enregistrer la photo de profil dans MongoDB
                self.save_profile_pic_to_mongodb()
                ##self.migrate_data_from_mysql_to_mongodb()
            except Exception as es:
                messagebox.showerror("Error", f"Due To:{str(es)}", parent=self.root)
    
    
    def save_profile_pic_to_mongodb(self):
        if self.profile_pic_path:
            # Convertir la photo de profil en bytes
            profile_pic_bytes = image_to_bytes(self.profile_pic_path)

            # Créer un document MongoDB pour la photo de profil
            profile_pic_data = {
                "enrollment": self.var_enrollment.get(),
                "profile_pic": profile_pic_bytes
            }

            # Insérer le document dans la collection MongoDB
            collection = db["profile_pics"]
            collection.insert_one(profile_pic_data)
    ...
    # Retrieve and display the profile picture from MongoDB
        profile_pic_data = db["profile_pics"].find_one({"enrollment": data[0]})
        if profile_pic_data:
            profile_pic = profile_pic_data["profile_pic"]
        # Display the profile picture in an image widget here
        ...
        if (self.var_enrollment.get() == "" or self.var_name.get() == "" or self.var_course.get() == ""):
            ...
            try:
                ...
                if update > 0:
                    ...
                    my_cursur.execute("update student set Name=%s,Course=%s,Department=%s,Year=%s,Semester=%s,Section=%s,Gender=%s,DOB=%s,Phone=%s,Email=%s,Address=%s where Enrollment=%s", (
                        self.var_name.get(),
                        self.var_course.get(),
                        self.var_department.get(),
                        self.var_year.get(),
                    ...
                    # Update the profile picture in MongoDB
                    if self.profile_pic_path:
                        profile_pic_bytes = image_to_bytes(self.profile_pic_path)
                        db["profile_pics"].update_one({"enrollment": self.var_enrollment.get()},
                                                    {"$set": {"profile_pic": profile_pic_bytes}}, upsert=True)
                ...
                messagebox.showinfo("Successful", "Student Information Updated Successfully...", parent=self.root)
            except Exception as es:
                messagebox.showerror("Error", f"Due To:{str(es)}", parent=self.root)
        ...
        if self.var_enrollment.get() == "":
            ...
            try:
                ...
                if Delete > 0:
                    ...
                    # Delete the profile picture from MongoDB
                    db["profile_pics"].delete_one({"enrollment": self.var_enrollment.get()})
    ...
    def fetch_data(self):
        ...
        if len(data) != 0:
            ...
            for i in data:
                enrollment = i[0]
                profile_pic_data = db["profile_pics"].find_one({"enrollment": enrollment})
                if profile_pic_data:
                    profile_pic = profile_pic_data["profile_pic"]
                    # Display the profile picture in an image widget here
            ...
            try:
                ...
                if len(data) != 0:
                    ...
                    for i in data:
                        ...
                        profile_pic_data = db["profile_pics"].find_one({"enrollment": enrollment})
                        if profile_pic_data:
                            profile_pic_bytes = profile_pic_data["profile_pic"]
                            self.display_profile_pic(profile_pic_bytes)  # Call the new method to display the profile picture
                            self.student_table.insert("", END, values=i)
                    conn.commit()
                conn.close()
            except Exception as es:
    ...
    def display_profile_pic(self, profile_pic_bytes):
            # Convert the bytes to a PhotoImage object
            img = Image.open(io.BytesIO(profile_pic_bytes))
            img = img.resize((798, 200), Image.LANCZOS)
            self.photoimg_7 = ImageTk.PhotoImage(img)

            # Update the existing Label with the new image
            self.right_img.configure(image=self.photoimg_7)
    ...
    def migrate_data_from_mysql_to_mongodb(myself):
        try:
            # Connexion à MySQL
            mysql_conn = mysql.connector.connect(host="localhost", user="root", password="root", database="student_management_system")
            mysql_cursor = mysql_conn.cursor()

            # Sélection des données à migrer depuis MySQL
            mysql_cursor.execute("SELECT * FROM student")
            data_to_migrate = mysql_cursor.fetchall()

            # Connexion à MongoDB
            mongodb_client = MongoClient("mongodb://localhost:27017/")
            mongodb_db = mongodb_client["nom_de_votre_base_de_donnees"]
            mongodb_collection = mongodb_db["student"]

            # Parcours des données et insertion dans MongoDB
            for row in data_to_migrate:
                ...
                mongodb_collection.insert_one(student_data)

            # Fermeture des connexions
            mysql_conn.close()
            mongodb_client.close()

            return True, "Migration des données réussie de MySQL vers MongoDB."

        except Exception as e:
            return False, f"Échec de la migration des données : {str(e)}"
